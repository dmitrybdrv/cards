...
export const useAuthForm = (validateFields: ValidateFieldsType[]) => {
    const { registration, login, authForgot, createPass, updateMe, logout } = useActions(authThunk)
    let {token} = useParams()
    ...
    return { register, handleSubmit, errors, onRegister, onLogin, onAuthForgot, onCreateNewPass, logout }
}
